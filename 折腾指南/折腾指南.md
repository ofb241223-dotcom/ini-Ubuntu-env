...
- [修复开机后终端/文件管理器打不开的问题](#修复开机后终端文件管理器打不开的问题)
...
1. **重新下载扩展**
   你可以打开 `./gnome_enabled_extensions.txt` 查看需要的扩展，并在“扩展管理器 (Extension Manager)”中重新下载它们。
2. **一键启用扩展**
   如果所有扩展都已下载完毕，可以用下面命令将它们全部启用：
   ```bash
   gsettings set org.gnome.shell enabled-extensions "$(cat ./gnome_enabled_extensions.txt)"
   ```
3. **恢复扩展的所有配置参数**
   ```bash
...
# 禁用可能产生冲突或不需要的其他快捷键（如图中的“复制为 HTML”、“全选”等）
