...
- [折腾指南](#折腾指南)
  ...
  - [Rime 输入法高级自定义 (雾凇拼音)](#rime-输入法高级自定义-雾凇拼音)
    - [1. 默认进入英文模式、使用英文标点、禁用表情](#1-默认进入英文模式使用英文标点禁用表情)
    - [2. 禁用 n 和 l 的模糊音](#2-禁用-n-和-l-的模糊音)
  - [Dialect 划词翻译交互脚本](#dialect-划词翻译交互脚本)
    - [1. 创建脚本文件](#1-创建脚本文件)
    - [2. 设置执行权限](#2-设置执行权限)
    - [3. 配置系统快捷键](#3-配置系统快捷键)
  - [Zsh 多行粘贴自动添加 \\ 拼接](#zsh-多行粘贴自动添加--拼接)
  - [好用的视频播放软件：Haruna](#好用的视频播放软件haruna)
  - [Flatpak 换源 (上海交大镜像)](#flatpak-换源-上海交大镜像)
  - [离线翻译软件：LocalTranslate](#离线翻译软件localtranslate)
  - [acer电脑调整CPU,GPU风扇转速,模式切换](#acer电脑调整cpugpu风扇转速模式切换)
---
...
如果你使用的是 Fcitx5 / iBus 框架下的 **雾凇拼音 (rime-ice)**，可以通过修改 `rime_ice.custom.yaml` 实现以下深度自定义。

### 1. 默认进入英文模式、使用英文标点、禁用表情

在 `patch:` 下方添加以下 `reset` 开关：
...
在 `speller/algebra` 部分找到对应的 `derive` 规则并注释掉（在行首加 `#`）：
...
> **注意**：修改完成后，务必在输入法菜单中点击 **「重新部署」 (Deploy)** 才能生效。

---

## Dialect 划词翻译交互脚本

Dialect 本身支持从选区翻译（`-n` 参数），为了实现“按下快捷键 -> 弹出菜单选择方向 -> 翻译”的闭环，可以使用以下脚本。

### 1. 创建脚本文件

**位置**：`~/.local/bin/dialect_translate.sh`

```bash
#!/bin/bash
# 弹出选择菜单
CHOICE=$(zenity --list --title="Dialect 划词翻译" --column="翻译方向" "英译中" "中译英" "自动检测模式")

if [ -z "$CHOICE" ]; then exit 0; fi

case $CHOICE in
  "英译中") flatpak run app.drey.Dialect -n -s en -d zh ;;
  "中译英") flatpak run app.drey.Dialect -n -s zh -d en ;;
  "自动检测模式") flatpak run app.drey.Dialect -n ;;
esac
```
...
```bash
chmod +x ~/.local/bin/dialect_translate.sh
```
...
在 **设置 -> 键盘 -> 自定义快捷键** 中添加：
* **名称**：`Dialect 交互翻译`
* **命令**：`/home/ofb77/.local/bin/dialect_translate.sh`
* **快捷键**：`Alt + E`
...
解决终端粘贴多行命令时，由于 zsh 的默认行为导致命令被直接分行执行的问题。
...
```zsh
function paste-join-lines() {
    local content
    # 提前定义一个包含真正换行符的变量
    local nl=$'\n'
    
    # 1. 读取粘贴板内容到变量 content
    zle .bracketed-paste content

    # 2. 去掉内容末尾多余的换行符（防止命令结尾出现悬挂的 \）
    content="${content%$nl}"

    # 3. 完美替换：将中间的换行符替换为 " \" + 真正的换行符
...
# 覆盖默认的粘贴处理函数
zle -N bracketed-paste paste-join-lines
```
...
Haruna 是一款基于 Qt 的开源视频播放器，界面简洁且功能强大。

* **安装方法**：在 Flathub 搜索下载，或使用命令：
  ```bash
...
如果你在安装 Flatpak 应用时下载速度过慢，可以使用上海交大的镜像源。
...
## 离线翻译软件：LocalTranslate

LocalTranslate 是一款可以在本地运行的翻译软件，保护隐私且非常稳定。

* **安装方法**：直接在 Flathub 搜索下载即可。

---

## acer电脑调整CPU,GPU风扇转速,模式切换

原生的Ubuntu并不支持切换模式,以及手动调整风扇转速,
![alt text](image-10.png)
即是我们在设置里切换了模式,但是并不会生效
好在有大佬在GitHub上开源了一款工具
[Div-Acer-Manager-Max](https://github.com/PXDiv/Div-Acer-Manager-Max?tab=readme-ov-file)
按照readme里面的教程安装即可解决